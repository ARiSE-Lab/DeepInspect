...
#datasets = ["coco","coco_gender","nus","cifar100","robust_cifar10_small","robust_cifar10_large","robust_cifar10_resnet","tiny_imagenet", "imagenet","imsitu"]
...
multi = ["coco","coco_gender","nus", "imsitu"]
for d in datasets:
    print("---------------------------")
    print("dataset: " + d)
    if d != "imsitu":
    ...
    if d == "coco" or d == "cifar100":
        ...
        distance_file = "/neuron_distance_from_predicted_labels_test_90.csv" # using neuron coverage threshold 0.5
        # neuron coverage threshold used in computing probability matrix can be selected based on file name
        # distance_file = "/neuron_distance_from_predicted_labels_test_90_0.6.csv" # using neuron coverage threshold 0.6
        ...
        for i in range(weight_matrix.shape[0]):
            for j in range(weight_matrix.shape[0]):
                if i == j:
                    continue
                if d in multi:
                    if (id2object[i],id2object[j]) in MODE_dist or (id2object[j],id2object[i]) in MODE_dist:
                        continue
                    MODE_dist[(id2object[i],id2object[j])] = dist[i][j]
                else:
                    if (str(i),str(j)) in MODE_dist or (str(j),str(i)) in MODE_dist:
                        continue
    ...
    for pair in d_test_confusion.keys():
        ...
        if (pair[1],pair[0]) in d_test_confusion:
            # mean
            n_test_confusion[pair] = (d_test_confusion[pair] + d_test_confusion[(pair[1],pair[0])])*1.0/2
        else:
            n_test_confusion[pair] = d_test_confusion[pair]

    for key, value in [(k, n_test_confusion[k]) for k in sorted(n_test_confusion, key=n_test_confusion.get, reverse=True)]:
        truth.append(key)
    ...
    for key, value in [(k, distance[k]) for k in sorted(distance, key=distance.get, reverse=False)]:
    ...
    if d != "imsitu":
        MODE_positive = []
        MODE_positive_value = []
        for key, value in [(k, MODE_dist[k]) for k in sorted(MODE_dist, key=MODE_dist.get, reverse=False)]:
    ...
    threshold = np.mean(truth_value) + np.std(truth_value) # ground truth threshold
    d_threshold = np.mean(our_positive_value) - np.std(our_positive_value) # mean-1std threshold
    if d != "imsitu":
        mode_d_threshold = np.mean(MODE_positive_value) - np.std(MODE_positive_value)
    ...
    print("count of ground truth buggy pairs if using 'mean-1std' as threshold " + str(truth_n))
    print("buggy pairs rate in ground truth if using 'mean-1std' as threshold : " + str(truth_n*1.0/total))
    ...
    if d != "imsitu":
        # print(MODE_positive[:pred_n])
        # print(temp_truth)
        for p in MODE_positive[:mode_pred_n2]:  # using dist < mean-1std as predictions
            if p in temp_truth:
                mode_tp += 1
            else:
                mode_fp += 1
    ...
    for j in x:
        ...
        if d != "imsitu":
            #print(MODE_positive[:pred_n])
            #print(temp_truth)
            for p in MODE_positive[:pred_n]: #using top (100*j)% as predictions
                if p in temp_truth:
                    mode_tp += 1
                else:
                    mode_fp += 1
...
for i in range(2):
    for j in range(4):
        ...
        if ind < len(datasets):
            ...
            ax.plot([c[dataset] for _ in range(10)], np.linspace(0, 1, 10), 'r--')
            ax.plot(x, o[dataset], label='optimal', linestyle=':', linewidth=8, color='green')
            ax.plot(x, r[dataset], label='random', linestyle='--', linewidth=8, color='blue')
            ax.plot(x, y[dataset], label='DeepInspect', linewidth=8, color='orange')
            if dataset != 'imsitu':
                ax.plot(x, m[dataset], label='MODE', linestyle='-.', linewidth=8, color='purple')
                ax.text(0.23, 0.5, 'Gain wrt MODE =' + f'{gain2[dataset] * 100:.1f}' + '%', size=30)
            ax.text(0.23, 0.6, 'Gain wrt random =' + f'{gain[dataset] * 100:.1f}' + '%', size=30)
