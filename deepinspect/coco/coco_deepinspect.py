...
def get_coverage_test():
    ...
    model.eval()
    t = tqdm(test_loader, desc = 'Activation')
    count = 0
...
def get_yhats_val():
    ...
    # Data samplers.
    val_data = CocoObject(ann_dir = ann_dir, image_dir = image_dir, 
        split = 'val', transform = val_transform)
    image_ids = val_data.image_ids 
    image_path_map = val_data.image_path_map
    #80 objects
    id2object = val_data.id2object
    id2labels = val_data.id2labels
    # Data loaders / batch assemblers.
...
def deepinspect(sample_10):
    ...
    with open('p_layer_coverage_predicted_90_0.5.pickle', 'wb') as handle:
        pickle.dump(p_layer_coverage, handle, protocol=pickle.HIGHEST_PROTOCOL)


    with open('p_layer_coverage_predicted_90_0.5.pickle', 'rb') as handle:
