...
def main():
    ...
    testP = os.path.join(args.expDir, 'val.csv')
    testData = np.loadtxt(testP, delimiter=',').reshape(-1, 3)
    accuracyP = os.path.join(args.expDir, 'score.csv')
    accuracyData = np.loadtxt(accuracyP, delimiter=',', 
                              dtype={'names': ('epoch', 'split', 'eval_score'), 
                                     'formats': ('i4', 'S5', 'f4')})
    ...
    eval_score = np.array([x[2] for x in accuracyData])
    ...
    plt.ylabel('Correct')
    plt.legend()
    err_fname = os.path.join(args.expDir, 'correct.png')
    ...
    trainEvalScore = eval_score[train_ids]
    testEvalScore = eval_score[test_ids]

    fig, ax = plt.subplots(1, 1, figsize = (6, 5))
    # plt.plot(trainI, trainErr, label='Train')
    plt.plot(trainEpochI, trainEvalScore, label = 'Train')
    plt.plot(testEpochI, testEvalScore, label = 'Test')
    plt.xlabel('Epoch')
    plt.xticks(np.arange(min(trainEpochI), max(trainEpochI) + 1, 1.0))
    plt.ylabel('eval_score')
    plt.legend()
    accuracy_fname = os.path.join(args.expDir, 'eval_score.png')
    plt.savefig(accuracy_fname)
    print('Created {}'.format(accuracy_fname))


    loss_err_fname = os.path.join(args.expDir, 'loss-error-eval_score.png')
